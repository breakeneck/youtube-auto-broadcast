...
→ `Scenario::startObs()` (SSH на 192.168.0.15, `systemctl --user start ffmpeg-cast`)
→ ffmpeg `/dev/video0` + мікрофон → rtmps YouTube (дефолтний стрім) → goLive → Telegram-нотифікація.
...
| OBS запущений / obs-start активний | OBS тримає `/dev/video0` — зупинити: `systemctl --user stop obs-start` (може зависнути — тоді `pkill -9 -x obs6`) |
...
- Ключові файли: `backend/server.php` (doStart/doStop/check-schedule + starting-лок),
  `backend/src/yuri/Scenario.php` (старти/стоп сценарію, пулінг active-стріму),
  `backend/src/yuri/Youtube.php` (API-обгортки), `backend/data/state.json` (id/scheduled_row/starting).